...
INSERT INTO[ORDERS].[ORDERS]([ORDER_ID],[CUSTOMER_ID],[BOOK_ID],[ORDER_DATE],[QUANTITY],[TOTAL_AMOUNT],[ORDER_STATUS])
VALUES 
(1141, 137,119,'08/09/2023',10,65.6, ' Delivered ' ),
(1142, 423,450,'03/09/2023',4,103.88, ' Delivered ' ),
(1143, 420,411,'09/09/2024',5,65.4, ' Delivered ' ),
(1144, 377,253,'09/05/2023',7,245.42, ' Delivered ' ),
(1145, 195,146,'10/08/2023',11,181.89, ' Delivered ' ),
(1146,120,313,'02/03/2024',8,172.48, ' Delivered ' ),
(1147, 488,250,'06/10/2023',9,76.95, ' Delivered ' ),
(1148,15,101,'07/11/2023',2,78.94, ' Delivered ' ),
(1149,110,146,'08/12/2024',6,98.94, ' Delivered ' ),
(1150,417,450,'01/02/2024',7,181.79, ' Delivered ' )

SELECT * FROM[BOOK].[BOOKS]

----WRITE A CTE TO SELECT BOOKS WITH PRICES HIGHER THAN 35 DOLLARS
;WITH CTE_MJ AS
(
SELECT [BOOK TITLE]=[TITLE],[BOOK_ID],[PRICE]= FORMAT([PRICE],'C0')
FROM[BOOK].[BOOKS]
WHERE [PRICE] > 35
)
...
(
SELECT [CATEGORY],[BOOK_ID],[PRICE]
FROM [BOOK].[BOOKS]
)
SELECT [CATEGORY],[QUANTITY SOLD]= COUNT ([QUANTITY]),[AVG PRICE]=FORMAT( AVG([PRICE]),'C0'),[TOTAL SALES]=SUM([TOTAL_AMOUNT])
FROM CTE_CATEGORY AS A
JOIN [ORDERS].[ORDERS] AS B
ON A.BOOK_ID= B.BOOK_ID
GROUP BY CATEGORY
